...
module Ecom
  ...
  def add
    ...
    LineItem.create! :purchase => @cart, :product => product, :price => product.price
  ...
  def get_cart_value
    @cart = session[:cart_id] ? Ecom::Purchase.find(session[:cart_id]) : current_user.Ecom::Purchase.new(params[:purchase])
  end
